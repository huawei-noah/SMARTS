...
from rllib_agent import RLLibTFSavedModelPolicy, TrainingModel
...
NUM_SOCIAL_VEHICLES = 10

ACTION_SPACE = gym.spaces.Box(
    low=np.array([0.0, 0.0, -1.0]), high=np.array([1.0, 1.0, 1.0]), dtype=np.float32,
)

NUM_NEIGHBORHOOD_VEHICLES = NUM_SOCIAL_VEHICLES + len(PREDATOR_IDS) + len(PREY_IDS) - 1
NEIGHBORHOOD_VEHICLE_STATES = gym.spaces.Dict(
    {
        "heading": gym.spaces.Box(low=0, high=2 * np.pi, shape=(1,)),
...
# In the code below, we reuse the AgentInterface instance created here.
# This serves as a test to verify that AgentInterface is indeed stateless.
shared_agent_interface = AgentInterface.from_type(AgentType.Standard)

rllib_agents = {}
for agent_id in PREDATOR_IDS:
    rllib_agents[agent_id] = {
        "agent_spec": AgentSpec(
            interface=shared_agent_interface,
            policy_builder=lambda: RLLibTFSavedModelPolicy(
