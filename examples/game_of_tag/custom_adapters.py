...
NEIGHBORHOOD_VEHICLE_STATES = gym.spaces.Dict(
    {
        ...
        "rel_position": gym.spaces.Box(low=-1e4, high=1e4, shape=(2,)),
    }
)
    ...
    states = [
        {
            ...
            "rel_position": np.array(v.position[:2]-ego_state.position[:2]), 
        }
        for v in nv_states
    ...
    states += [
        {
            ...
            "rel_speed": np.array([0]),
            "rel_position": np.array([10000, 10000]),
        }
