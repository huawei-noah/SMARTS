...
class Reward(gym.Wrapper):
    ...
    def _reward_with_goal(self, obs: Observation, env_reward: np.float64, sim) -> float:
        reward = 0
        ...
        goal_road = sim._scenario.road_map.nearest_lane(goal_position).road
        vehicle_road = sim._scenario.road_map.nearest_lane(vehicle_position).road

        route_length = sim._scenario.road_map.generate_routes(start_road=vehicle_road, end_road=goal_road).distance_between(vehicle_position, goal_position)
