...
from smarts.env.utils.action_conversion import get_formatters, ActionOptions
...
def display_spaces():
    ...
    for name, action_type in ActionSpaceType.__members__.items():
        ...
        print()
        print(f"- For {tn} {(ActionOptions.multi_agent, ActionOptions.full, ActionOptions.default)} -")
        print(f"Gym space = {formatted.space!r}")
        print()

def display_agent_type_spaces():
    method_name, _, _ = f"{AgentInterface.from_type=}".partition("=")
    print(f"Note that `{method_name}` generates a pre-configured agent type with an existing action space.")
