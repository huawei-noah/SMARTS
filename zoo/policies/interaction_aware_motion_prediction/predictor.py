...
class Decoder(nn.Module):
    def __init__(self, modalities=3):
        ...
        decoder = nn.Sequential(nn.Dropout(0.1), nn.Linear(256, 128), nn.ELU(), nn.Linear(128, 30*3))
    ...
    def forward(self, hidden, state):
        ...
        for i in range(self.modalities):
            ...
            score = self.scorers[i](torch.cat([hidden, self.encode(pred.view(-1, N, 90).detach())], dim=-1))
