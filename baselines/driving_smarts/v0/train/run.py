...
from train import env as competition_env
...
def main(args: argparse.Namespace):
    ...
    envs_train = {}
    envs_eval = {}
    wrappers = competition_env.wrappers(config=config)
    for scen in config["scenarios"]:
        envs_train[f"{scen}"] = competition_env.make(
            config=config, scenario=scen, wrappers=wrappers
        )
        envs_eval[f"{scen}"] = competition_env.make(
            config=config, scenario=scen, wrappers=wrappers
        )
