...
class MainNet(nn.Module):
    ...
    def compute_loss(self, nn_outputs, labels):
        dx_loss = self.criterion(nn_outputs['dx'], labels[:,0].view(labels.shape[0], 1))
        dy_loss = self.criterion(nn_outputs['dy'], labels[:,1].view(labels.shape[0], 1))
        dh_loss = self.criterion(nn_outputs['d_heading'], labels[:,2].view(labels.shape[0], 1))
        total_loss = dx_loss + dy_loss + dh_loss

        return total_loss, dx_loss, dy_loss, dh_loss

def save_losses(train_total_losses, train_total_loss, 
                train_dx_losses, train_dx_loss, 
                train_dy_losses, train_dy_loss, 
                test_total_losses, test_total_loss,
                test_dx_losses, test_dx_loss, 
                test_dy_losses ,test_dy_loss):
    ...
    return train_total_losses, train_dx_losses, train_dy_losses, test_total_losses, test_dx_losses, test_dy_losses

def train(model, dataset_path, checkpoint_path, cache=False, lr=0.001, num_epochs=100, batch_size=32, save_steps=30):
    """
    ...
    train_loader = create_datasets(dataset_path, save_path=checkpoint_path, cache=cache, batch_size=batch_size)  # read datasets
    ...
    for epoch in range(num_epochs):
        ...
        epochs.append(epoch)
        train_total_loss, train_dx_loss, train_dy_loss = train_epoch(model, loss_function, optimizer, train_loader)
        test_total_loss = 0
        test_dx_loss = 0
        test_dy_loss = 0

        train_total_losses, train_dx_losses, train_dy_losses, test_total_losses, test_dx_losses, test_dy_losses = save_losses(train_total_losses, train_total_loss, 
                                                                                                                                train_dx_losses, train_dx_loss, 
                                                                                                                                train_dy_losses, train_dy_loss, 
                                                                                                                                test_total_losses, test_total_loss,
                                                                                                                                test_dx_losses, test_dx_loss, 
...
def train_epoch(model, loss_function, optimizer, data_loader):
    ...
    print('Train Loss: {:.4f}'
          .format(total_loss))
    print('Train dx ;oss: {:.4f}'
          .format(total_dx_loss))
    print('Train dy loss: {:.4f}'
          .format(total_dy_loss))
    print('Train dh loss: {:.4f}'
          .format(total_dh_loss))      
...
def main():
    ...
    print('Training...')
    m = MainNet()
    train(m, args.dataset_path, args.output_path, cache=args.cache, lr=args.learning_rate, num_epochs=args.num_epochs, batch_size=args.batch_size, save_steps=args.save_steps)
    print('Training Done.')
