...
### Added
### Changed
- Changed the `ParallelEnv` to support passing in the `polling_period` parameter.
### Deprecated
### Fixed
- Fixed `seed()` method in `HiWayEnv` to confirm to the `gym.Env` prototype: it now returns a list of seeds.
### Removed
### Security
