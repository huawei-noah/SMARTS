...
import numpy as np
from ultra.baselines.dqn.network import *
from smarts.core.agent import Agent
from ultra.utils.common import merge_discrete_action_spaces, to_3d_action, to_2d_action
import pathlib, os
from ultra.baselines.dqn.network import DQNWithSocialEncoder
from ultra.baselines.dqn.explore import EpsilonExplore
...
class DQNPolicy(Agent):
    ...
    def step(
        self, state, action, reward, next_state, done, max_steps_reached, others=None
    ):
...
class BehavioralDQNPolicy(DQNPolicy):
    def __init__(
        ...
        # super(BehavioralDQNPolicy, self).__init__(*args, **kwargs)
        # policy params
        current_dir = os.path.dirname(os.path.realpath(__file__))
        policy_params = load_yaml(f"{current_dir}/params.yaml")
