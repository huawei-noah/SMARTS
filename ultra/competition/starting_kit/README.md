...
- `interface`:
  ...
  The vector observation adapter requires the car to have a NeighborhoodVehicles sensor that senses all nearby vehicles within a certain `radius`, and a Waypoints sensor that produces waypoints up to a certain `lookahead` distance along the vehicle's mission path. The values of 200.0 and 20 for the `radius` and `lookahead`, respectively, can
  be changed. However, these are the values that will be used for the evaluation on CodaLab, so it is recommended that these values be used. If your agent does not have these values for the `radius` and `lookahead` when sumitted for evaluation on CodaLab, the evaluation will fail.
